...
#######################################################################################################################
    ...
    def attack(self):
        ...
        if self.id in surr or self.id == center:
...
class node:
    ...
    def add_neibour(self, grid, evade=False, atk=False , directed=False , diameter=None):
        ...
        if evade and atk:
            ...
            radius = (diameter - 1) / 2 # 3 => 1 , 5 => 2

            radius = (diameter - 1) # 3 => 2 => , 5 => 4
            ...
            while  grid[d_id].row < 14 and grid[d_id + 15].obj_type ==  '.' and down: #DOWN
                down -= 1
...
class Gameboard:
    ...
    def bomb_detonate(self , unit): #IF MY UNIT AREN'T THERE DETONATE 
        ...
        for bomb in this_bomb:
            ...
            if bomb.expires - self.tick_number < 25:
    ...
    #     # self.p[unit].safe['safe'] = False
    #     # self.p[unit].safe['target_list'] = []
    #     # self.p[unit].safe['safe_spot'] = None
    ...
    #     #     if 2*self.h(i , self.p[unit]) -1 < i.blast_diameter:
    ...
    def AI(self , unit_id):
        ...
        print("ULTIMATE SAFE SPOT " , evade)
        ...
        #     action , unit_id = self.catch_spans(unit_id=unit_id)
            ...
            if action != None:
                ...
                if self.p[unit_id].eInfo['attack']:
                    if self.p[unit_id].eInfo['eNeigh'].id == self.p[unit_id].id:
                    # if self.p[unit_id].eInfo['attack']:
