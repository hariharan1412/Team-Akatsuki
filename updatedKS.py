...
#######################################################################################################################
    ...
    def soldier(self):
        ...
        print(" ENEIBOUR : " , self.eInfo['eNeigh'].id)
        ...
        if self.eInfo['attack']:
            ...
            print(" INSIDE ATTACK TRUE : " , self.gameBoard.can_i(self.unit_id))
...
class node:
    ...
    def add_neibour(self, grid, evade=False, atk=False , directed=False , diameter=None , move=False , fire=False , friendly_unit=False):
        ...
        if atk:
            ...
            if self.row > 0 and (grid[self.id - 15].obj_type == '.' or grid[self.id - 15].obj_type == 'fp' or grid[self.id - 15].obj_type == 'bp') and grid[self.id - 15].player == None:  # UP
                eNeigh.append(grid[self.id - 15])

            if self.row < 14 and (grid[self.id + 15].obj_type == '.' or  grid[self.id + 15].obj_type == 'fp'  or  grid[self.id + 15].obj_type == 'bp' ) and grid[self.id + 15].player == None:  # DOWN
                eNeigh.append(grid[self.id + 15])

            if self.col > 0 and ( grid[self.id - 1].obj_type == '.'  or grid[self.id - 1].obj_type == 'fp' or grid[self.id - 1].obj_type == 'bp') and grid[self.id - 1].player == None:  # LEFT
                eNeigh.append(grid[self.id - 1])

            if self.col < 14 and (grid[self.id + 1].obj_type == '.' or grid[self.id + 1].obj_type == 'fp' or grid[self.id + 1].obj_type == 'bp') and grid[self.id + 1].player == None: # RIGHT
                eNeigh.append(grid[self.id + 1])
...
class Gameboard:
    ...
    def catch_spans(self, unit_id):
        ...
        if self.p[unit_id].title == 'SOLIDER':
            return None
    ...
    def AI(self, unit_id):
        ...
        if escape != None: #TODO : NEED TO CHECK 
            ...
            action , unit_id = self.path_finding(unit=unit_id , end_point=evade  , escape=True)
        ...
        # elif spawns and self.p[unit_id].title != 'SOLDIER':
            ...
            if self.p[unit_id].role:
                ...
                print(' ACTION BEFORE ***', self.p[unit_id].goto)


                action, unit_id = self.p[unit_id].role()
                
                print(' ACTION AFTER ***', self.p[unit_id].goto)
