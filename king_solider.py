...
#######################################################################################################################
    ...
    def whoseThr(self, node, l, who=False):
        ...
        for i in range(start, end+1, 15):  # 97  -  127 , 15 => O(1) #82 , 142 , 15
            for x in range(i-l, i+l+1):  # 96   -  99 # 80 
    ...
    def coreAtk(self):
        ...
        if self.id in surr or self.id == self.center or self.breakWood:
            ...
            fxf = self.whoseThr(self.center, 2, True) # OBS , ENEMY 
            ...
            action, unit_id = self.gameBoard.path_finding(unit=self.unit_id, end_point=self.goto[1])
    ...
    def soldier(self):
        ...
        if self.eInfo['attack']:  #TODO : NEED TO ADD BOMB CHECK 

            can_i = True

            for i in self.gameBoard.myTeam:

                print("in for loop : " ,i)
                # if i != unit:
                evade = self.gameBoard.bomb_evade( unit=i , end_node=self.id)
...
class Gameboard:
    ...
    def update_board(self, gameState, tick_number=None, call=True):
        ...
        if tick_number < 2 and call and self.bara == 1:
            ...
            for i in self.myTeam:
                ...
                temp = self.path_finding(unit=i, end_point=self.p[i].center, team7=True)

                #TODO : NEED TO ADD RANDOM VALUE
                lowCost.append(temp)
            ...
            d = dict(zip(lowCost, self.myTeam)) #COST KEY => TEAMMATE VALUE
            
            self.p[d[low]].setRole('king') #LOWEST UNIT GETS KEY #SET ROLE TO THIS UNIT
        ...
        # for i in self.grid:
        #     print(i.id , i.obj_type , i.weight , i.override)
        # self.override = None
    ...
    def path_finding(self, unit, end_point, check=False, team7=False):
        ...
        if end.obj_type == 'm' or  end.obj_type == 'x' or end.obj_type == 'b' :  # NEED TO ADD DIAGNOL NEIBOUR # USE WHILE
            ...
            # self.p[unit].goto[-1] = end.id #TODO : NEED TO CHECK 
