...
class player:
    ...
    def __init__(self , grid , Gameboard , agent_id , unit_id , hp):
        ...
        self.center = 112
        # self.goto = [112 , None] # self.grid[77].`obj_type == 'b' => self.grid[77].obj_type == '.'
        self.goto = [None , 112] # self.grid[77].`obj_type == 'b' => self.grid[77].obj_type == '.'
        # self.goto = {
            # '0' : self.center,
            # '1' : None
...
#######################################################################################################################
    ...
    def monitor(self):
        ...
        if len(self.goto) > 1:
            if self.goto[1] == self.id:
                # self.goto.pop()
                self.goto[1] = None

        print(" POGA VENDIYA IDAM " ,self.goto)
    ...
    def updEn(self, unit): #UNIT => ENEMY
        ...
        neibour = self.grid[eId].add_neibour(self.grid, evade=True, atk=True) #EMPTY SPOTS 
            ...
            for i in self.gameBoard.eTeam: # C , E , G
                ...
                if temp > manHat:
                    ...
                    try:
                        ...
                        self.gameBoard.currAtks[e] = self.unit_id # ENEMY => TEAMATE
                        break
...
#######################################################################################################################
    ...
    def move_bro(self , f_unit, path ): #f_unit => FRIEND UNIT
        ...
        while q:
            ...
            if a in path: 
                ...
                neibours = a.add_neibour(self.grid , move=True , f_unit=f_unit) #GET ALL EMPTY NODE  
        ...
        # action , unit = self.path_finding(unit=unit , end_point=a.id , check=True)
        action , unit = self.gameBoard.path_finding(unit=self.unit_id , end_point=a.id , check=True)

        return action , unit , f_unit
...
class node:
    ...
    def add_neibour(self, grid, evade=False, atk=False , directed=False , diameter=None , move=False , f_unit=None , fire=False):
        ...
        if evade and atk:
            ...
            # if self.row > 0 and grid[self.id - 15].obj_type ==  '.' and  grid[self.id - 15].player != f_unit and not grid[self.id - 15].enemy: #UP
            #     evade_neibours.append(grid[self.id - 15])

            # if self.row < 14 and grid[self.id + 15].obj_type ==  '.' and  grid[self.id + 15].player != f_unit and not grid[self.id + 15].enemy: #DOWN
            #     evade_neibours.append(grid[self.id + 15])

            # if self.col > 0 and grid[self.id -1].obj_type ==  '.'  and  grid[self.id - 1].player != f_unit and not grid[self.id - 1].enemy: #LEFT
            #     evade_neibours.append(grid[self.id - 1])
            
            # if self.col < 14 and grid[self.id + 1].obj_type ==  '.' and  grid[self.id + 1].player != f_unit and not grid[self.id + 1].enemy: #RIGHT
            #     evade_neibours.append(grid[self.id + 1])        
...
class Gameboard:
    ...
    def AI(self , unit_id):
        ...
        # elif evade != None:
            ...
            #     print("WHAT IS IN : ",self.p[unit_id].goto)
        ...
        #     action , unit_id = self.catch_spans(unit_id=unit_id)
            ...
            if detonate_action != None:
                ...
                if self.p[unit_id].eInfo['attack']:
                    ...
                    print('SATTI THALAYA ' , self.p[unit_id].goto)
